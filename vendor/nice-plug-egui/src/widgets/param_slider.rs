...
use egui::{
    self, Key, Response, Sense, Stroke, TextEdit, TextStyle, Ui, Vec2, Widget, WidgetText, emath,
    vec2,
};
    ...
    fn slider_ui(&mut self, ui: &Ui, response: &mut Response) {
        ...
        if ui.is_rect_visible(response.rect) {
            ...
            if filled_proportion > 0.0 {
                ...
                let filled_bg = if response.hovered() {
            ...
            ui.painter().rect_stroke(
                response.rect,
                0.0,
                Stroke::new(1.0f32, ui.visuals().widgets.active.bg_fill),
                egui::StrokeKind::Middle,
            );
        }
    }

    fn value_ui(&mut self, ui: &mut Ui) {
