        ...
        extern "C-unwind" fn $sel(this: &NSView, _: Sel, event: &NSEvent){
            ...
            if let Some(key_event) = state.process_native_key_event(event){
                ...
                if let EventStatus::Ignored = status {
                    unsafe {
                        // Start lookup at the genuine NSView, not
                        // `this.class().superclass()`. Under host KVO/CoreAnimation
                        // runtime subclassing the leaf class's superclass is this
                        // baseview class itself, so deriving the super class from it
                        // re-enters this method and recurses until the stack overflows.
                        let () = msg_send![super(this, NSView::class()), $sel:event];
                    }
                }
            }
        }
...
extern "C-unwind" fn hit_test(this: &NSView, _sel: Sel, point: NSPoint) -> Option<&NSView> {
    // SAFETY: Our superclass is NSView. Start lookup at the genuine NSView
    // rather than `this.class().superclass()`: when the host installs a runtime
    // subclass (Bitwig adds a KVO observer, reparenting the instance to
    // NSKVONotifying_BaseviewNSView_…), the leaf's superclass is permanently this
    // baseview class, so `super` re-enters this method and recurses until the
    // stack guard page faults (SIGSEGV) on the first pointer/hit-test event.
    let super_result: Option<&NSView> =
        unsafe { msg_send![super(this, NSView::class()), hitTest: point] };
...
extern "C-unwind" fn view_will_move_to_window(
    ...
    unsafe {
        // Genuine NSView, not the runtime leaf's superclass — see hit_test.
        let () = msg_send![super(this, NSView::class()), viewWillMoveToWindow: new_window];
    }
}
